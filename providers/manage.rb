    ...
    search(new_resource.data_bag, "groups:#{new_resource.search_group} AND NOT action:remove") do |u|
      ...
      if home_dir != "/dev/null"
        ...
        if u['ssh_private_key']
          keys = Chef::EncryptedDataBagItem.load("keys", "#{u['username']}")
          key_type = keys['ssh_private_key'].include?("BEGIN RSA PRIVATE KEY") ? "rsa" : "dsa"
          template "#{home_dir}/.ssh/id_#{key_type}" do
            ...
            group u['gid'] || u['id']
            mode "0400"
            variables :private_key => keys['ssh_private_key']
          end
        end

        if u['ssh_public_key']
          keys = Chef::EncryptedDataBagItem.load("keys", "#{u['username']}")
          key_type = keys['ssh_public_key']['ssh_public_key'].include?("ssh-rsa") ? "rsa" : "dsa"
          template "#{home_dir}/.ssh/id_#{key_type}.pub" do
            ...
            group u['gid'] || u['id']
            mode "0400"
            variables :public_key => keys['ssh_public_key']
          end
        end
      end
    end
